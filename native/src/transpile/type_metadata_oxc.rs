...
mod tests {
    use super::super::type_metadata::{
        render_type_metadata_for_test, TypeMetadataParitySnapshot as Snapshot,
    };
    use super::*;
    use crate::closure_metadata::{
        ClosureEnumDeclaration, ClosureTypeDeclaration, ClosureTypeReference, ClosureTypeSymbol,
    };
    ...
    fn symbol(
        ...
        ClosureTypeSymbol {
            builtin_name: builtin_name.map(str::to_string),
            declaration_file_path: None,
            declaration_id: None,
            declaration_start: None,
            diagnostic_name: id.rsplit(':').next().unwrap_or(id).to_string(),
            id: id.to_string(),
            kind: kind.to_string(),
    ...
    fn reference(token: &str, symbol_id: &str) -> ClosureTypeReference {
    ...
    fn metadata() -> ClosureFileMetadata {
        ClosureFileMetadata {
            ambient_globals: Vec::new(),
            annotations: vec![
                ClosureAnnotation {
                    references: vec![
                        reference("__WIDGET__", "runtime:Widget"),
                        reference("__IMPORTED__", "runtime:Imported"),
                    ],
                    target: ClosureAnnotationTarget::Binding {
                        binding_name: "useWidget".to_string(),
                    },
                    template: "/** @param {!__WIDGET__} widget @return {!__IMPORTED__} */\n"
                        .to_string(),
                    type_bearing: true,
                },
                ClosureAnnotation {
                    references: vec![reference("__STRING__", "builtin:string")],
                    target: ClosureAnnotationTarget::Member {
                        member_kind: "field".to_string(),
                        member_name: "value".to_string(),
                        owner_binding_name: "Widget".to_string(),
                        is_static: false,
                    },
                    template: "/** @type {__STRING__} */\n".to_string(),
                    type_bearing: true,
                },
                ClosureAnnotation {
                    references: Vec::new(),
                ...
                ClosureAnnotation {
                    ...
                    target: ClosureAnnotationTarget::Member {
                        member_kind: "method".to_string(),
                        member_name: "left".to_string(),
                        owner_binding_name: "pair".to_string(),
                        is_static: false,
                    },
                    template: "/** @return {number} */\n".to_string(),
                    type_bearing: true,
            ...
            declarations: vec![ClosureTypeDeclaration {
                declared_symbol_id: "type:Config".to_string(),
                exported: false,
                id: "decl:Config".to_string(),
                references: vec![reference("__WIDGET__", "runtime:Widget")],
                template: "/** @record */\nfunction Config() {}\nif (false) {\n  /** @type {!__WIDGET__} */\n  Config.prototype.widget;\n}\n".to_string(),
            }],
            ...
            file_path: "fixture.js".to_string(),
            runtime_module_id: None,
            source_file_path: "fixture.js".to_string(),
            symbols: vec![
                symbol("runtime:Widget", "runtime", Some("Widget"), None),
                symbol("runtime:Imported", "runtime", Some("Imported"), None),
                symbol("builtin:string", "builtin", None, Some("string")),
                symbol("type:Config", "type", None, None),
            ],
        }
    }
    ...
    fn oxc_snapshot(source: &str, metadata: &ClosureFileMetadata) -> Snapshot {
        let allocator = Allocator::default();
        let parsed = Parser::new(&allocator, source, SourceType::mjs()).parse();
        ...
        let bound = BoundTypeMetadata::bind(&parsed.program, &identity, Some(metadata), true);
        let runtime_names = runtime_type_names_from_program(&parsed.program, &identity, &bound);
        let mut fresh_names = FreshNameAllocator::from_program(&parsed.program, &identity);
        let mut prepared = bound.prepare(&mut fresh_names, &runtime_names, Some(2));
        ...
        for statement in parsed.program.body {
            match statement {
                ...
                statement if statement.as_declaration().is_some() => code.push(
                    prepared
                        .render_statement(&identity, statement, &[])
                        .unwrap(),
                ),
                _ => {}
            }
    ...
    #[test]
    fn statement_delivery_matches_swc_structure_and_counts() {
        let source = r#"
            import { Imported } from "./dep";
            class Widget { value; read() { return 1; } }
            function useWidget(widget) { return widget; }
            const pair = { left() { return 1; } };
        "#;
        let metadata = metadata();
        let swc = render_type_metadata_for_test(source, &metadata, Some(2)).unwrap();
        let oxc = oxc_snapshot(source, &metadata);

        assert_eq!(oxc.declaration_lines, swc.declaration_lines);
        assert_eq!(oxc.enum_name, swc.enum_name);
        assert_eq!(
            normalize(&oxc.code),
            normalize(&swc.code),
            "swc:\n{}\noxc:\n{}",
        ...
        assert_eq!(oxc.delivery.counts, swc.delivery.counts);
        assert_eq!(oxc.delivery.diagnostics, swc.delivery.diagnostics);
        assert!(oxc.code.contains("@param {!Widget}"), "{}", oxc.code);
        assert!(oxc.code.contains("@return {!Imported}"), "{}", oxc.code);
        assert!(oxc.code.contains("Widget.prototype.value"), "{}", oxc.code);
        assert_eq!(oxc.code.matches("value;").count(), 1, "{}", oxc.code);
        assert_eq!(oxc.delivery.counts.annotationCount, 1);
        assert_eq!(oxc.delivery.counts.memberAnnotationCount, 3);
        assert_eq!(oxc.delivery.counts.typeDeclarationCount, 1);
        assert_eq!(oxc.delivery.counts.enumDeclarationCount, 1);
    }

    #[test]
    fn declaration_rename_uses_symbols_and_zero_based_oxc_spans() {
        let rendered = rename_declaration_template(
            "/** @record */\nfunction Config() {}\nfunction shadow(Config) { return Config; }\nConfig.prototype.value;\n",
            "Config",
            "Config$$type$$2",
        )
        .unwrap();
        assert!(
            rendered.contains("function Config$$type$$2()"),
            "{rendered}"
        );
        assert!(
            rendered.contains("Config$$type$$2.prototype.value"),
            "{rendered}"
        );
        assert!(
            rendered.contains("shadow(Config) { return Config; }"),
            "{rendered}"
        );
    }

    #[test]
    fn symbol_keys_survive_hoist_renames_without_remapping_metadata() {
        let allocator = Allocator::default();
        let parsed = Parser::new(&allocator, "class Widget { value; }", SourceType::mjs()).parse();
        assert!(parsed.diagnostics.is_empty(), "{:#?}", parsed.diagnostics);
        ...
        metadata.annotations.retain(|annotation| {
            matches!(
                &annotation.target,
                ClosureAnnotationTarget::Member { member_kind, member_name, .. }
                    if member_kind == "field" && member_name == "value"
            )
        });
        metadata.declarations.clear();
        metadata.enums.clear();
        metadata
            .symbols
            .retain(|symbol| matches!(symbol.id.as_str(), "runtime:Widget" | "builtin:string"));
        let bound = BoundTypeMetadata::bind(&program, &identity, Some(&metadata), true);

        let Statement::ClassDeclaration(class) = &mut program.body[0] else {
            panic!("expected class");
        };
        let binding = class.id.as_mut().expect("named class");
        let key = identity.key_of_binding(binding);
        binding.name = Ident::from_in("Widget$$2", &allocator);
        identity.rename(key, Ident::from_in("Widget$$2", &allocator));

        let runtime_names = runtime_type_names_from_program(&program, &identity, &bound);
        let mut fresh_names = FreshNameAllocator::from_program(&program, &identity);
        let mut prepared = bound.prepare(&mut fresh_names, &runtime_names, Some(2));
        let code = prepared
            .render_statement(&identity, program.body.pop().unwrap(), &[])
            .unwrap();
        assert!(code.contains("Widget$$2.prototype.value"), "{code}");
        assert_eq!(prepared.finish().counts.memberAnnotationCount, 1);
    }

    #[test]
    fn duplicate_top_level_names_fail_closed_like_swc() {
        let source = "function item() {}\nfunction item(value) { return value; }\n";
        let mut metadata = metadata();
        metadata.annotations = vec![ClosureAnnotation {
            references: Vec::new(),
            target: ClosureAnnotationTarget::Binding {
                binding_name: "item".to_string(),
            },
            template: "/** @return {number} */\n".to_string(),
            type_bearing: true,
        }];
        ...
        assert_eq!(oxc.delivery.diagnostics, swc.delivery.diagnostics);
        assert_eq!(
            oxc.delivery.diagnostics[0].reason,
            "annotation-target-not-found"
        );
    }
}
