...
impl HoistPlan {
    ...
    /// A resolved binding can be referenced directly from `consumer_module_id`
    /// when its owner is hoisted into the same chunk.
    pub(super) fn is_direct_binding(
        ...
        self.is_hoisted(&binding.owner_module_id)
            ...
            && self.chunk_of(&binding.owner_module_id) == self.chunk_of(consumer_module_id)
    }
        ...
        for edge in &scan.import_edges {
            if edge.namespace {
                match (&edge.namespace_members, consumer_hoisted) {
                    (Some(members), true) => {
                        let same_chunk_direct = plan.is_hoisted(&edge.target_module_id)
                            && plan.chunk_of(&edge.target_module_id).is_some()
                            && plan.chunk_of(&edge.target_module_id) == plan.chunk_of(module_id);
                        if same_chunk_direct {
                            // Members rewrite to direct bindings, except when
                            // they resolve to a cross-chunk owner, where the
