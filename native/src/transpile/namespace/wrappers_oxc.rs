...
mod tests {
    ...
    fn normalize_binding_map<T: Clone + Ord>(
        ...
        for (binding, value) in values {
            normalized
                .entry(identity.symbol(*binding).to_string())
                .or_default()
                .push(value.clone());
        }
    ...
    fn facts(
        program: &Program<'_>,
        identity: &ModuleIdentity,
    ) -> super::super::wrappers::DynamicImportWrapperFacts {
        ...
        super::super::wrappers::DynamicImportWrapperFacts {
            function_wrappers: normalize_binding_map(&wrappers.function_wrappers, identity),
            object_wrappers: normalize_binding_map(&wrappers.object_wrappers, identity),
            object_function_wrappers: normalize_binding_map(
                &wrappers.object_function_wrappers,
                identity,
            ),
            object_carriers: normalize_binding_map(&object_carriers, identity),
            promise_carriers: normalize_binding_map(&promise_carriers, identity),
        }
    }
    ...
    #[test]
    fn wrappers_objects_promises_and_invalidations_match_swc() {
        let source = r#"
const loadA = () => __dynamicImport("A");
function loadB() { return __dynamicImport("B"); }
const directObject = { a: loadA, b: () => __dynamicImport("B") };
const mergedObjects = [directObject, { c: () => __dynamicImport("C") }];
function getObject() { let temporary; return directObject; }
const getObjectArrow = () => getObject();
const objectCarrier = getObjectArrow();
const indexedObject = mergedObjects[0];
const promiseA = loadA();
const promiseB = directObject.b();
const promiseEither = flag ? loadA() : loadB();
const promiseFromObject = indexedObject.a();
const promiseCell = slot();
inspect(promiseCell);
write(promiseCell, loadB());
const objectCell = box();
inspect(objectCell);
write(objectCell, directObject);
let invalidPromise = loadA();
invalidPromise += 1;
let destructured = loadB();
[destructured] = values;
function shadow(__dynamicImport) {
  const ignored = () => __dynamicImport("X");
  return ignored;
}
"#;
        ...
        let swc = super::super::wrappers::collect_dynamic_import_wrapper_facts_for_test(source);
        assert_eq!(oxc, swc);

        assert_eq!(oxc.function_wrappers.get("loadA"), Some(&vec![ids(&["A"])]));
        assert_eq!(oxc.function_wrappers.get("loadB"), Some(&vec![ids(&["B"])]));
        assert!(!oxc.function_wrappers.contains_key("ignored"));
        assert_eq!(
            oxc.promise_carriers.get("promiseEither"),
            Some(&vec![ids(&["A", "B"])])
        );
        assert_eq!(
            oxc.promise_carriers.get("promiseCell"),
            Some(&vec![ids(&["B"])])
        );
        assert!(!oxc.promise_carriers.contains_key("invalidPromise"));
        assert!(!oxc.promise_carriers.contains_key("destructured"));
        assert!(oxc.object_function_wrappers.contains_key("getObject"));
        assert!(oxc.object_function_wrappers.contains_key("getObjectArrow"));
        assert!(oxc.object_carriers.contains_key("objectCarrier"));
        assert!(oxc.object_carriers.contains_key("indexedObject"));
        assert!(oxc.object_carriers.contains_key("objectCell"));
    }
}
