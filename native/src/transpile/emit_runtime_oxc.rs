...
#[cfg(test)]
mod tests {
    use super::super::emit_runtime::emit_runtime_text_for_test;
    use super::*;
    use crate::closure_metadata::{
        ClosureAnnotation, ClosureAnnotationTarget, ClosureFileMetadata,
    };
    ...
    fn context(
        workspace_dir: &Path,
        slots: HashMap<String, BundlerModuleSlots>,
    ) -> TranspileContext {
        TranspileContext {
    ...
    fn slots(names: &[&str]) -> BundlerModuleSlots {
        BundlerModuleSlots::from_export_names(
            &names
                .iter()
    ...
    fn metadata() -> ClosureFileMetadata {
        ClosureFileMetadata {
            ...
            file_path: "entry.js".to_string(),
            runtime_module_id: None,
            source_file_path: "entry.js".to_string(),
            symbols: Vec::new(),
        }
    }
    ...
    fn oxc_emit(
        ...
        emit_bundler_runtime_module_text(
            &allocator,
            file_path,
            &mut program,
            &identity,
    ...
    fn root(name: &str) -> std::path::PathBuf {
        let root =
            std::env::temp_dir().join(format!("gcc-emit-runtime-{}-{name}", std::process::id()));
    ...
    #[test]
    fn runtime_text_matches_swc_for_imports_exports_namespace_and_metadata() {
        let root = root("module");
        let entry = root.join("entry.js");
        let dep = root.join("dep.js");
        let star = root.join("star.js");
        std::fs::write(&dep, "export default 0; export const named = 1;").unwrap();
        std::fs::write(&star, "export const alpha = 1; export const beta = 2;").unwrap();
        let source = r#"
            import defaultValue, { named as imported } from "./dep.js";
            import * as ns from "./dep.js";
            export const stable = imported;
            export const shape = { imported };
            export let live = defaultValue;
            live++;
            export function read() { return ns.named + live; }
            export { imported as alias };
        ...
        let star_id = to_goog_module_id(&star, &root);
        let mut context = context(
            ...
            HashMap::from([
                ...
                (dep_id, slots(&["default", "named"])),
                (star_id, slots(&["default", "alpha", "beta"])),
            ]),
        );
        context.type_metadata_enabled = true;
        let metadata = metadata();
        let swc =
            emit_runtime_text_for_test(&entry, source, &context, Some(&metadata), None).unwrap();
        let oxc = oxc_emit(&entry, source, &context, Some(&metadata));
        assert_eq!(
            normalize(&oxc.code),
            normalize(&swc.code),
            "swc:\n{}\noxc:\n{}",
        ...
        let named_slot = context
            .bundler_module_slots
            .values()
            .find_map(|slots| slots.slot_for("named"))
            .unwrap();
        assert!(
            oxc.code.contains(&format!("ns[{named_slot}]")),
            "{}",
            oxc.code
        );
    ...
    fn anonymous_default_forms_match_swc_structure() {
        ...
        let entry = root.join("entry.js");
        let entry_id = to_goog_module_id(&entry, &root);
        let context = context(&root, HashMap::from([(entry_id, slots(&["default"]))]));
        for source in [
            "export default function() { return 1; }",
            "export default class { method() { return 1; } }",
            "export default makeValue();",
        ] {
            std::fs::write(&entry, source).unwrap();
            let swc = emit_runtime_text_for_test(&entry, source, &context, None, None).unwrap();
            let oxc = oxc_emit(&entry, source, &context, None);
            assert_eq!(
                normalize(&oxc.code),
                normalize(&swc.code),
                "source:\n{source}\nswc:\n{}\noxc:\n{}",
    ...
    #[test]
    fn commonjs_and_lazy_namespace_facades_match_swc_structure() {
        let root = root("commonjs");
        let entry = root.join("entry.js");
        let source = "const cjsValue = 1;";
        std::fs::write(&entry, source).unwrap();
        let entry_id = to_goog_module_id(&entry, &root);
        let mut context = context(
            &root,
            HashMap::from([(entry_id.clone(), slots(&["default", "cjsValue"]))]),
        );
        context.lazy_target_module_ids.insert(entry_id);
        let swc =
            emit_runtime_text_for_test(&entry, source, &context, None, Some("cjsValue")).unwrap();
        let allocator = Allocator::default();
        let (mut program, identity) = parse(&allocator, source);
        let oxc = emit_bundler_runtime_module_text(
            &allocator,
            &entry,
            &mut program,
            &identity,
        ...
        assert!(oxc.code.contains(".__esModule = true"), "{}", oxc.code);
        std::fs::remove_dir_all(root).unwrap();
    }

    #[test]
    fn dynamic_import_callback_namespace_rewrite_matches_swc() {
        let root = root("dynamic");
        let entry = root.join("entry.js");
        let dep = root.join("dep.js");
        std::fs::write(&dep, "export const named = 1;").unwrap();
        let entry_id = to_goog_module_id(&entry, &root);
        let dep_id = to_goog_module_id(&dep, &root);
        let runtime_dep_id = to_bundler_runtime_module_id(&dep_id);
        let source = format!(
            "const load = () => __dynamicImport({runtime_dep_id:?}); export function run() {{ return load().then(ns => ns.named); }}"
        );
        std::fs::write(&entry, &source).unwrap();
        let mut context = context(
            &root,
            HashMap::from([
                (entry_id, slots(&["run"])),
                (dep_id.clone(), slots(&["named"])),
            ]),
        );
        context
            .bundler_runtime_logical_ids
            .insert(runtime_dep_id, dep_id);
        let swc = emit_runtime_text_for_test(&entry, &source, &context, None, None).unwrap();
        let oxc = oxc_emit(&entry, &source, &context, None);
        assert_eq!(
            normalize(&oxc.code),
        ...
        assert!(oxc.code.contains("ns[0]"), "{}", oxc.code);
        std::fs::remove_dir_all(root).unwrap();
    }

    #[test]
    fn reflective_namespace_operations_fail_closed_like_swc() {
        let root = root("namespace-error");
        let entry = root.join("entry.js");
        let dep = root.join("dep.js");
        std::fs::write(&dep, "export const named = 1;").unwrap();
        let source = "import * as ns from './dep.js'; export const names = Object.keys(ns);";
        std::fs::write(&entry, source).unwrap();
        let entry_id = to_goog_module_id(&entry, &root);
        let dep_id = to_goog_module_id(&dep, &root);
        let context = context(
            &root,
            HashMap::from([(entry_id, slots(&["names"])), (dep_id, slots(&["named"]))]),
        );
        let swc_error =
            emit_runtime_text_for_test(&entry, source, &context, None, None).unwrap_err();
        let allocator = Allocator::default();
        let (mut program, identity) = parse(&allocator, source);
        let oxc_error = emit_bundler_runtime_module_text(
            &allocator,
            &entry,
            &mut program,
            &identity,
        ...
        assert!(oxc_error.contains("reflective Object.* operations"));
        std::fs::remove_dir_all(root).unwrap();
    }

    #[test]
    fn generated_runtime_globals_rename_without_touching_local_shadows() {
        let root = root("runtime-shadow");
        let entry = root.join("entry.js");
        let source = r#"
            function local(__require) { return __require; }
            const load = () => __require("manual");
            export { local, load };
        "#;
        std::fs::write(&entry, source).unwrap();
        let entry_id = to_goog_module_id(&entry, &root);
        let context = context(
            &root,
            HashMap::from([(entry_id, slots(&["local", "load"]))]),
        );
        let swc = emit_runtime_text_for_test(&entry, source, &context, None, None).unwrap();
        let oxc = oxc_emit(&entry, source, &context, None);
        assert_eq!(
        ...
        assert!(oxc.code.contains("function(__require_1,"), "{}", oxc.code);
        assert!(
            oxc.code.contains("function local(__require)"),
            "{}",
            oxc.code
        );
        assert!(oxc.code.contains("__require(\"manual\")"), "{}", oxc.code);
        std::fs::remove_dir_all(root).unwrap();
    }

    #[test]
    fn synthesized_runtime_placeholders_use_the_allocated_parameter_name() {
        let allocator = Allocator::default();
        let source = "function local(__require) { return __require; } const value = 0;";
        let (mut program, identity) = parse(&allocator, source);
        let Statement::VariableDeclaration(declaration) = &mut program.body[1] else {
            panic!("expected variable declaration");
        };
        declaration.declarations[0].init = Some(Expression::new_identifier(
            SPAN,
            "__require",
            &AstBuilder::new(&allocator),
        ));
        let (names, _) = RuntimeBindingNames::allocate(&allocator, &mut program, &identity);
        assert_eq!(names.require, "__require_1");
        let code = Codegen::new().build(&program).code;
        assert!(code.contains("const value = __require_1"), "{code}");
        assert!(code.contains("local(__require)"), "{code}");
    }
