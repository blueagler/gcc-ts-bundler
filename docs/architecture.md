...
Closure Compiler 20260726 rejects private class elements even with `language_in=UNSTABLE` and `language_out=ECMASCRIPT_NEXT`. As an upstream capability workaround, the Oxc stage structurally detects modules containing private identifiers and enables its class-properties transform only for those modules, before Closure sees them on every target. Generated instance-field slots and method/accessor brands are rewritten from opaque `WeakMap`/`WeakSet` allocations to module-local Symbol-backed slots: ordinary enumeration, spread, and JSON omit them, `symbol in receiver` supplies the brand test, and Closure can rename and inline the small slot operations and module-scope private functions. Oxc's exact static-private class check/storage shape remains the per-form faithful fallback so derived constructors are still rejected; authored `new WeakMap`/`new WeakSet` expressions are span-pinned and never rewritten. Explicit `Object.getOwnPropertySymbols` reflection, descriptors, symbol copying/deletion, prototype forgery, and proxy traps can observe or forge the lowered instance slots; those explicit-reflection/metaprogramming cases are accepted TypeScript-output-contract divergences, while ordinary private access order, `TypeError` brand failures, `#x in` RHS validation, and weak reachability remain covered by regressions. Private accessor assignments use a real object-literal setter in the helper runtime, so Closure renames the generated assignment and setter key together under ADVANCED rather than disconnecting a quoted `defineProperty` key from a renamed member access. The helper surface is module-local Closure input and fails closed on reserved-binding collisions or any unowned Oxc helper; modules without private elements keep the unchanged transform path, including hash-prefixed text inside Lit CSS templates. Retire this workaround when the pinned Closure release accepts private elements directly and the regression corpus passes without it.
