...
Historical implementation plan with partial landing notes, not a current task
list or API contract. Later work changed the pipeline and source layout; retain
the measurements and rejected alternatives as evidence. See
[Architecture](../development/architecture.md) and [Workflows](../development/workflows.md) for
...
- **`compiler.externs` is live**, not discarded. `resolveCompilerExterns` in
  ...
  `docs/reference/vite.md` treat this as the explicit-externs path.
...
The resident CLI worker already reuses one JVM for sequential argv jobs.
Constructing `CompilerOptions` directly is an API capability, not a demonstrated
build-time win. In `v20260909`, `--num_parallel_threads` is consumed for parsing
and dependency prebuilding, but not type checking or optimization. The default
is now capped at four available CPUs, with an explicit override for contention.
CHECKS snapshots contain the whole checked program, so JS or extern
edits invalidate them, and restoring them does not replay check diagnostics.
Identical programs already use our output cache. See the
[current build-time measurements](optimization-architecture.md#v20260909-build-time-verification)
before adding a second cache or changing the driver.
...
JSDoc brands get lossy. CHECKS snapshots require an unchanged checked JS/extern
world; they are not an incremental-checking shortcut after ordinary edits.
JVM warmth and bounded parallel parsing are already available through the
resident CLI worker. The measured CLI setup cost does not justify an API
rewrite for build time alone; see the current measurements above.
