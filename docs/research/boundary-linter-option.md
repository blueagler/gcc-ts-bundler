...
**Decision: no-go for now.** The `lint` subcommand and flags below are a proposal,
not supported CLI syntax. The existing [Vite build report](../reference/vite.md#build-report)
is the shipped evidence surface; see §5 for the conditions for reconsideration.
