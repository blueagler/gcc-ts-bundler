...
class Generator {
    ...
    if (ts.isObjectLiteralExpression(exportsExpr)) {
      ...
      for (const property of exportsExpr.properties) {
        if (ts.isShorthandPropertyAssignment(property)) {
          ...
          let identifier: null | ts.Identifier = null;
   ...
   */
  private extractTsmesStatement(): TsmesCallBreakdown | undefined {
  ...
  private maybeExtractTypeName(cast: ts.AsExpression): null | ts.Identifier {
    if (
  ...
  generateExportShimJavaScript(): [string, FileSummary] {
    ...
    fileSummary.addStrongRequire({ name: "goog", type: Type.CLOSURE });
 ...
 * 'Local'}.
 */
type GoogExports = Map<string, string> | string;
