...
export interface GoogModuleProcessorHost {
   ...
   */
  transformDynamicImport: "closure" | "nodejs";
 ...
 *   require('foo')
 */
function extractRequire(call: ts.CallExpression): null | ts.StringLiteral {
 ...
 */
function literalTypeOfSymbol(symbol: ts.Symbol): boolean | string | undefined {
  ...
  if (hasMultipleProvides) {
    ...
    reportDiagnostic(
      ...
      `referenced JavaScript module ${
        tsImport
      } provides multiple namespaces and cannot be imported by path.`,
    );
 ...
 */
function isExportsAssignment(expr: ts.Expression): null | string {
 ...
 */
function rewriteCommaExpressions(expr: ts.Expression): null | ts.Statement[] {
...
export function commonJsToGoogmoduleTransformer(
  ...
  return (context: ts.TransformationContext): ts.Transformer<ts.SourceFile> => {
    ...
    context.onSubstituteNode = (hint, node: ts.Node): ts.Node => {
      ...
      let importExportDecl: ts.ExportDeclaration | ts.ImportDeclaration;
    ...
    return (sf: ts.SourceFile): ts.SourceFile => {
      ...
      ): null | RewrittenExportsAssignment {
       ...
       * them.
       */
      function rewriteDynamicRequire(node: ts.Node): null | ts.Node {
        // Look for  `???( ??? )`
        if (!ts.isCallExpression(node) || node.arguments.length !== 1) {
          return null;
        }

        let importedUrl: null | ts.StringLiteral = null;
