...
export function getGeneratedExterns(
  externs: { [fileName: string]: { moduleNamespace: string; output: string } },
  rootDir: string,
): string {
...
export function generateExterns(
...
): { diagnostics: ts.Diagnostic[]; moduleNamespace: string; output: string } {
  let output = "";
  ...
  function isFirstValueDeclaration(decl: ts.DeclarationStatement): boolean {
    ...
    return (
      earlierDecls.length === 0 || earlierDecls.every(ts.isVariableDeclaration)
    );
  }
  ...
  function writeVariableDeclaration(
    ...
    if (decl.name.kind === ts.SyntaxKind.Identifier) {
      ...
      emit(
        jsdoc.toString([{ tagName: "type", type: mtt.typeToClosure(decl) }]),
      );
      emit("\n");
  ...
  function writeEnum(
    ...
    for (const member of decl.members) {
      ...
      if (!memberName) {
        members += `  /* TODO: ${ts.SyntaxKind[member.name.kind]}: ${escapeForComment(
          member.name.getText(),
        )} */\n`;
        continue;
      }
  ...
  function visitor(node: ts.Node, namespace: ReadonlyArray<string>) {
    ...
    switch (node.kind) {
      ...
      default:
        emit(
          `\n// TODO(tsickle): ${ts.SyntaxKind[node.kind]} in ${debugLocationStr(
            node,
            namespace,
          )}\n`,
        );
