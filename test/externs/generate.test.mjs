...
test("barrier accounting counts every extern property shape", async () => {
  ...
  expect(accounting.propertyNames).toEqual([
    "flat two",
    "flatOne",
    "ownerOne",
    "recordOne",
...
test("isPlatformBuiltin matches Node and Bun builtins", () => {
  expect(isNodeBuiltin("fs")).toBe(true);
  expect(isPlatformBuiltin("fs")).toBe(true);
  expect(isPlatformBuiltin("node:fs")).toBe(true);
  expect(isPlatformBuiltin("bun:ffi")).toBe(true);
  expect(isPlatformBuiltin("bun-types")).toBe(true);
  expect(isPlatformBuiltin("contract-pkg")).toBe(false);
});

test.serial(
  "generateExterns defaults to package-local declaration crawl",
  async () => {
    const fixture = await createExternFixture();
...
test.serial(
  "collectReachableTypeFiles does not follow or report platform specifiers",
  async () => {
    const fixture = await createFixture();
    await fixture.write(
      "types/app.d.ts",
    ...
    await collectReachableTypeFiles({
      compilerOptions,
      entryFiles: [path.join(fixture.projectRoot, "types/app.d.ts")],
      includeDependencies: true,
      onUnresolved: (specifier) => {
        unresolved.push(specifier);
      },
    });
    expect(unresolved).toContain("missing-pkg");
    expect(unresolved).not.toContain("node:fs");
    expect(unresolved).not.toContain("fs");
    expect(unresolved).not.toContain("bun:ffi");
  },
);

test.serial(
  "generateExterns accepts multiple package modules in one call",
  async () => {
    const fixture = await createExternFixture();
    await fixture.write(
      "node_modules/other-pkg/package.json",
      JSON.stringify({ name: "other-pkg", types: "./index.d.ts" }),
    );
    await fixture.write(
    ...
    const result = await generateExternsFromSource({
      ...
      mode: "boundary-aware",
      modules: [
        { exports: "used", runtime: "external", specifier: "contract-pkg" },
        { exports: "used", runtime: "external", specifier: "other-pkg" },
      ],
      projectRoot: fixture.projectRoot,
      srcDir: fixture.srcDir,
    });
    ...
    expect(
      result.typedDeclarations.moduleExports
        ...
        .sort(),
    ).toEqual(["contract-pkg", "other-pkg"]);
  },
);
