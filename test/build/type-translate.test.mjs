...
import {
  ...
  toClosureType,
} from "../../src/build/transpile/type-metadata/metadata/type-render/index.ts";
...
test("a degraded generic target drops its type arguments (`?<...>` is a syntax error)", async () => {
  const { applyTypeArguments } = await import(
    "../../src/build/transpile/type-metadata/metadata/type-render/index.ts"
  );
...
test("heritage strips `!` and refuses supertypes with no nominal form", async () => {
  const { buildClassJsDoc } = await import(
    "../../src/build/transpile/type-metadata/metadata/docs.ts"
  );
